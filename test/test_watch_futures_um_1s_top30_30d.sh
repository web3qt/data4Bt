...
WATCH_SCRIPT="$PROJECT_ROOT/watch-futures-um-1s-top30-30d"
...
cat > "$TMP_DIR/state/symbol_progress.json" <<'EOF'
{
  "AAAUSDT": {
    ...
    "progress": 100,
    "status": "completed",
    "last_update": "2026-03-11T11:00:00+08:00",
    "worker_id": 0
  },
  "BBBUSDT": {
    "symbol": "BBBUSDT",
    ...
    "progress": 40,
    "status": "running",
    "last_update": "2026-03-11T11:01:00+08:00",
    "worker_id": 2
  },
  "CCCUSDT": {
    "symbol": "CCCUSDT",
    ...
    "progress": 13.33,
    "status": "failed",
    "last_update": "2026-03-11T11:02:00+08:00",
    "worker_id": 4
  }
}
EOF
...
case "$query" in
  *"WHERE symbol IN ('AAAUSDT','BBBUSDT','CCCUSDT')"*)
    printf '123456\t2\t2026-02-09 00:00:00.000\t2026-03-10 23:59:59.000\n'
    ;;
  *)
...
There is a screen on:
	63213.futures_um_1s_top30_30d	(Detached)
1 Socket in /tmp/.screen.
OUT
EOF
chmod +x "$TMP_DIR/fake_screen_ls.sh"

cat > "$TMP_DIR/futures_um_1s_top30_30d.log" <<'EOF'
line 1
line 2
EOF
...
output="$(
  ...
  LOG_FILE="$TMP_DIR/futures_um_1s_top30_30d.log" \
  SCREEN_NAME="futures_um_1s_top30_30d" \
  TOTAL_SYMBOLS="30" \
...
assert_contains "$output" "Runner: RUNNING (screen: futures_um_1s_top30_30d)" "runner status"
...
assert_contains "$output" "worker=2 symbol=BBBUSDT progress=40.00% current_month=2026-02" "running symbol line"
assert_contains "$output" "line 2" "recent log"

echo "test_watch_futures_um_1s_top30_30d.sh: PASS"
