...
func TestBinanceDownloader_BuildDownloadURL_FuturesUM(t *testing.T) {
	downloader := NewBinanceDownloader(
		config.BinanceConfig{
			BaseURL:    "https://data.binance.vision",
	...
	got := downloader.BuildDownloadURL("BTCUSDT", time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC))
...
func TestBinanceDownloader_MarketTypeExchangeInfoURL(t *testing.T) {
	spot := NewBinanceDownloader(
		config.BinanceConfig{MarketType: "spot"},
		config.DownloaderConfig{},
	)
	if spot.exchangeInfoURL != "https://api.binance.com/api/v3/exchangeInfo" {
		t.Fatalf("spot exchangeInfoURL mismatch: %s", spot.exchangeInfoURL)
	}

	um := NewBinanceDownloader(
		config.BinanceConfig{MarketType: "futures_um"},
		config.DownloaderConfig{},
	)
	if um.exchangeInfoURL != "https://fapi.binance.com/fapi/v1/exchangeInfo" {
		t.Fatalf("futures_um exchangeInfoURL mismatch: %s", um.exchangeInfoURL)
	}

	cm := NewBinanceDownloader(
		config.BinanceConfig{MarketType: "futures_cm"},
		config.DownloaderConfig{},
	)
	if cm.exchangeInfoURL != "https://dapi.binance.com/dapi/v1/exchangeInfo" {
		t.Fatalf("futures_cm exchangeInfoURL mismatch: %s", cm.exchangeInfoURL)
	}
}
...
func TestBinanceDownloader_GetAvailableDates(t *testing.T) {
	// 模拟规则：2023-12 及之后的月度文件可用
	monthRE := regexp.MustCompile(`-(\d{4}-\d{2})\.zip$`)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "HEAD" {
			t.Errorf("Expected HEAD request, got %s", r.Method)
		}

		matches := monthRE.FindStringSubmatch(r.URL.Path)
		if len(matches) == 2 && matches[1] >= "2023-12" {
			w.WriteHeader(http.StatusOK)
		} else {
			w.WriteHeader(http.StatusNotFound)
		}
